...
# TODO
# functions to test on all dataset : get epe and u's
...
def get_gflops(): # TODO test with my trained model
    # raft = RAFT(iters=4, iters_pred=4)
    # raft.load_weights('checkpoints/model')
    # flops = get_flops(raft, batch_size=1)
    # print(f"FLOPS: {flops / 10 ** 9:.03} G")
    pass


def get_footprint(model): #TODO test while doing full training on cloud (summary in train)
    # total_parameters = 0
...
def augmenta_test():
    ...
    raft.load_weights('checkpoints/model')
    total_parameters = 0
    cap = cv2.VideoCapture('training/datasets/Augmenta/RGB_mini.mp4')
    # width  = cap.get(cv2.CAP_PROP_FRAME_WIDTH)  
    # height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
    out = cv2.VideoWriter(
        filename='results/visualisation_optical_flow.mp4',
    ...
    while(True):
        ...
        if np.any(frame)==None:
            np.savetxt("results/photometric_loss.csv", np.array(loss_list), delimiter=",")
            cap.release()
            out.release()
