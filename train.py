...
from tf_raft.datasets import FlyingChairs, MpiSintel, KITTI, ShapeSetter, CropOrPadder
from tf_raft.training import VisFlowCallback, LearningRateLoggingCallback, first_cycle_scaler


def train_chairs(config, logdir):
    try:
...
def train_sintel(config, logdir):
    try:
        data_config = config['data']
        root = data_config['root']
    ...
    raft.load_weights(logdir+'/checkpoints/model')
    print('done')
...
if __name__ == '__main__':
    ...
    with open('training/configs/train_kitti.yml', 'r') as f:
        config = yaml.load(f, Loader=yaml.SafeLoader)
    logd = config['logdir']
    logdir = os.path.join(logd, datetime.now().strftime("%Y-%m-%dT%H-%M"))
    if not os.path.exists(logdir):
